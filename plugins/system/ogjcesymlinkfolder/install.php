 ...
 */
class plgSystemOgjcesymlinkfolderInstallerScript
{
	/**
	...
	function preflight($type, $parent) 
	{
	    ...
	    {
	        JFactory::getApplication()->enqueueMessage(JText::sprintf('PLG_OGJCESYMLINKFOLDER_PHP_VERSION_WARNING', PHP_VERSION),'WARNING');
	    }
	    
	    // Joomla! broke the update call, so we have to create a workaround check.
	    $db = \Joomla\CMS\Factory::getDbo();
	    $db->setQuery("SELECT extension_id, enabled FROM #__extensions WHERE element = 'ogsymlinkfolder'");
	    ...
	    if (!$this->enabled) {
	        ...
	        if (version_compare(JVERSION, '3.9.0', '<')  && $type !="uninstall") {
	            JFactory::getApplication()->enqueueMessage('Cette version du plugin ogsymlinkfolder nécessite une version de Joomla 3.9.0 et suivantes.<br/>Ce plugin ne pourra pas fonctionner correctement. Faite une mùise à jour de Joomla avant d\'installer cette mise à jour du plugin.','WARNING');
	            return false;
	        }
	    }
		return;
	...
	public function PluginActivation($type, $parent)
	{
		if($this->newinstall){
		    $db = JFactory::getDbo();
			$db->setQuery("SELECT extension_id, enabled FROM #__extensions WHERE element = 'ogjcesymlinkfolder'");
			$result = $db->loadObject();
			if($result){
				$this->extension_id = $result->extension_id;
				$this->enabled = $result->enabled;
		...
		// End of CSS Styling
		if (!$this->newinstall) { 
			$inst_text = JText::sprintf('PLG_OGJCESYMLINKFOLDER_INST_VERSION_UPRG',$parent->manifest->version); 
		} 
		else {  
			$inst_text = JText::sprintf('PLG_OGJCESYMLINKFOLDER_INST_VERSION',$parent->manifest->version);
		}
		...
		echo "<div class='gski_install'>
				...
				<div class='installed'>
					<ul>
						<li>ogJCESymLinkFolder</li>
					</ul>
				</div>

				<div class='proceed'>
					<ul>
						<li><a href='index.php?option=com_plugins&view=plugins&filter[search]=id:".$this->extension_id ."' alt='Configuration du plugin OGJCESymLinkfolder'><br/> Configuration</a><br/></li>
					</ul>
				</div>
				<div class='proceed'>".JText::sprintf('PLG_OGJCESYMLINKFOLDER_INST_VERSION_INFO')."
				</div>";
	}
